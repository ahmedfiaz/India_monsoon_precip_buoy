...
#### sat. vap pressure using era5 formula ###

def era5_es(temp):
    '''
    Use era5 sat. vap. pressure calculation:
    https://www.ecmwf.int/sites/default/files/elibrary/2016/17117-part-iv-physical-processes.pdf#subsection.7.4.2
    sat. vap. pressure wrt water for T>0C, wrt ice for T<-23C and a linear interpolation for temps. in between
    '''
    tmelt  = 273.15
    tempc = temp - tmelt # in C
    Tice=250.16 #K
    ...
    cond=np.logical_and(temp<tmelt,temp>Tice)
    alpha[cond]=(temp[cond]-Tice)**2/(tmelt-Tice)**2
    
    return alpha*esw+(1-alpha)*esi
...
def era5_esw(temp):
    ...
    return a1*np.exp(a3*(temp-T0)/(temp-a4))
...
#### END sat. vap pressure using era5 formula ###
...
def qs_calc(press_hPa, temp):
    ...
    #get es in Pa
    es = es_calc(temp) * 100.
#     es = era5_es(temp) ### uses sat. vap pressure over ice and water
